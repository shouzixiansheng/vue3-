  ...
  <div>
    <el-card class="box-card">
        ...
        <div class="card-header">
          <div style="margin-left: 30px">
            <el-form :inline="true" :model="formInline" class="">
              <el-form-item label="邮箱搜索:" style="margin-right: 30px">
              <span>
                <el-input v-model="formInline.email" placeholder="请输入搜索内容" style="width: 150px"/>
              </span>
              </el-form-item>
      ...
      <div v-loading="loading">
        <el-table :data="emailData" border>
          <el-table-column label="邮箱号">
            <template #default="scope">
              <div>
                <span style="margin-right: 20px">{{ scope.row.email }}</span>
              </div>
            </template>
          </el-table-column>
...
const onSubmit = () => {
  if (formInline.value.email.length !== 0){
    verifyParam.email = formInline.value.email
  }
  getVerifyCodeAdminQuery(verifyParam)
}
const onReset = () => {
  formInline.value.email = ''
  verifyParam = {
    page: 1,
    size: pagination.pageSize,
    code_type: 'EMAIL'
